...
- All four slots are parallel-parking spaces. Their map-owned body-center goal
  yaw is `0` on both rows. Reverse parking is expressed by `direction=REVERSE`,
  independently from body yaw; the selector converts the goal pose to the
  planner's rear-axle reference point.
