...
class HybridAStarPlanner:
    ...
    def __init__(self, map_cfg: dict, vehicle_cfg: dict, planner_cfg: dict):
        ...
        requested_area_ids = set(str(v) for v in collision.get("drivable_area_ids", []))
        self.drivable_polygons: List[List[Tuple[float, float]]] = []
        for area in map_cfg.get("drivable_areas", []):
            area_id = str(area.get("id", ""))
            if requested_area_ids and area_id not in requested_area_ids:
                continue
            poly = area.get("planner_polygon_m") or area.get("raw_polygon_m")
            if not poly:
                continue
            self.drivable_polygons.append([(float(p[0]), float(p[1])) for p in poly])
        if self.require_drivable_area and not self.drivable_polygons:
            raise ValueError("require_drivable_area=true but map_v1 has no matching drivable polygon")
    ...
    @staticmethod
    def _point_on_segment(x: float, y: float, a: Tuple[float, float], b: Tuple[float, float], eps: float = 1e-9) -> bool:
        ax, ay = a
        bx, by = b
    ...
    @classmethod
    def _point_in_polygon(cls, x: float, y: float, polygon: Sequence[Tuple[float, float]]) -> bool:
        if len(polygon) < 3:
            return False
        inside = False
        j = len(polygon) - 1
        for i in range(len(polygon)):
            xi, yi = polygon[i]
            xj, yj = polygon[j]
            if cls._point_on_segment(x, y, (xj, yj), (xi, yi)):
                return True
            if (yi > y) != (yj > y):
    ...
    def point_is_drivable(self, x: float, y: float) -> bool:
        if not self.require_drivable_area:
            return True
        return any(self._point_in_polygon(x, y, poly) for poly in self.drivable_polygons)
    ...
    def point_collision(self, x: float, y: float, obstacles: Sequence[RectObstacle]) -> bool:
        ...
        if x < 0.0 or x > self.map_width or y < 0.0 or y > self.map_height:
            return True
        if not self.point_is_drivable(x, y):
            return True
