...
def write_debug_svg(path: Path, map_cfg: dict, slot_states: Dict[str, str], selected: SlotPlan, start: Pose) -> None:
    ...
    chunks.append(f'<text x="{margin}" y="{h-8:.1f}" font-size="15" fill="#a00">CAD map background active. OFFLINE PROTOTYPE: collision/drivable semantics are not fully classified yet.</text>')
    chunks.append('</svg>')
